...
from CODES.MOT_files.kd import ExcelProcessor_v2
from CODES.MOT_files.keydecoder_deduceMOT import AttributeDataProcessorMOT
...
class MOT_ETL:
    ...
    def process_data(self):
        ...
        if filtered_sheets:
            ...
            unique_product_codes = set(self.raw_data['Product Code'])
            unique_product_codes.discard(None)
            unique_product_codes.discard('')

            self.product_data = self.product_data[self.product_data['Product Code'].isin(unique_product_codes)]
    ...
    def RawData(self, sheet_name):
        ...
        deducer = AttributeDataProcessorMOT()
        ...
        # respondent_columns = [col for col in raw.columns if col == 'Product Code']
        #
        # if len(respondent_columns) > 1:
        #     raw['Purchase Intent_PI'] = raw['Purchase Intent_PI'].apply(lambda row: ''.join(row.dropna().astype(str)), axis=1)
    ...
    def Project(self):
        self.project_data = DataFrameEnhancerMOT(None, self.filepath, self.country_of_test, self.start_date,
    ...
    def get_date_range(self, raw):
        ...
        if 'Date' not in raw.columns:
            print(self.filepath,"----Error:'Date' column not found.")
    ...
    def reversal(self):
        ...
        unique_product_codes = set(raw_df['Product Code'])
        unique_product_codes.discard(None)
        unique_product_codes.discard('')
        self.product_data = self.product_data[self.product_data['Product Code'].isin(unique_product_codes)]


        # print("the quiire ",unique_product_codes)


        self.key_decoder = processed_df
        self.raw_data = raw_df
        self.consumer_data = consumer_df
        # print(tabulate(self.key_decoder, headers='keys', tablefmt='fancy_grid', showindex=True))
        # print(tabulate(self.project_data, headers='keys', tablefmt='fancy_grid', showindex=True))
        # print(tabulate(self.product_data, headers='keys', tablefmt='fancy_grid', showindex=True))
        # print(tabulate(self.raw_data.head(5), headers='keys', tablefmt='fancy_grid', showindex=True))
        # print(tabulate(self.consumer_data.head(5), headers='keys', tablefmt='fancy_grid', showindex=True))
...
class DescriptionProcessor:
    ...
    def process_dataframe(self, df):
        ...
        filtered_df = df[df['Column'].str.contains('|'.join(keywords), case=False, na=False) & df['Description'].str.contains('=', na=False)]
...
if __name__ == "__main__":
    directory_path = r"C:\Users\Z19661\PycharmProjects\LegacyData_\InputFiles\MOT\Adult Sparkling\Coke_Adult_Sparkling_SSD_Spain_v1_011924.xlsx"
    MOT_ETL(directory_path)
